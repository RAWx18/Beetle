...
class LiteLLMProvider(BaseLLM):
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            gemini_models = ["gemini/gemini-1.5-flash", "gemini/gemini-pro"]
            
            for model in gemini_models:
                try:
                    test_request.model = model
                    await self.generate(test_request)
                    logger.info(f"Health check passed with model: {model}")
                    return True
                except Exception as e:
                    logger.warning(f"Health check failed for model {model}: {str(e)}")
                    continue
            
            logger.error("All Gemini models failed health check")
            return False
