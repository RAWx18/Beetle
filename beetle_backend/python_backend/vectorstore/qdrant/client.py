    ...
    async def _create_enhanced_indexes(self) -> None:
        ...
        try:
            ...
            indexes = [
                ...
                ("filename", "text")  # Full-text search
            ]
            ...
            for field, schema in indexes:
                try:
                    ...
                    logger.warning(f"Failed to create index for {field}", error=str(e))
                    
        except Exception as e:
            logger.warning("Failed to create enhanced indexes", error=str(e))
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            if self.collection_name in [col.name for col in collections.collections]:
                collection_info = await self.client.get_collection(self.collection_name)
                self._is_healthy = collection_info.status == UpdateStatus.OK
            else:
                self._is_healthy = False
